...
if (isGroup && !msg.key.fromMe) {
  try {
    await antispamCmd.checkSpam(sock, msg, {
      from,
      sender,
      isGroup,
      groupMetadata,
...
if (isGroup && !msg.key.fromMe) {
  try {
    await antibadwordCmd.checkBadword(sock, msg, {
      from,
      sender,
      isGroup,
      groupMetadata,
    ...
    console.error('[Antibadword Check Error]', e);
  }
}
...
if (isGroup && !msg.key.fromMe) {
  try {
    await antistickerCmd.checkSticker(sock, msg, {
      from,
      sender,
      isGroup,
      groupMetadata,
    ...
    console.error('[Antisticker Check Error]', e);
  }
}
...
if (action === 'add' && groupSettings.welcome) {
  try {
    const welcomeMsg = `*_@${displayName} joined the group, welcome! 🎉_*`;
    ...
    console.error("Welcome Error:", err);
      ...
      // --- GOODBYE/LEFT LOGIC ---
        ...
        const finalMessage = `@${displayName} *_has left the group, we will miss them! 👋_*`;
